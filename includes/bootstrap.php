...
function event_validation(array $input, bool $admin = false): array
{
    ...
    if ($data['title'] === '' || mb_strlen($data['title']) > 255)
        $errors[] = 'Enter a title of up to 255 characters.';
    if ($data['description'] === '' || mb_strlen($data['description']) > 5000)
        $errors[] = 'Enter a description of up to 5,000 characters.';
    if ($data['venue'] === '' || mb_strlen($data['venue']) > 255)
        $errors[] = 'Enter a valid venue.';
    if ($data['city'] === '' || mb_strlen($data['city']) > 100)
        $errors[] = 'Enter a valid city.';
    if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $data['event_date']) || $data['event_date'] < date('Y-m-d'))
        $errors[] = 'The event date must be today or later.';
    if (!preg_match('/^\d{2}:\d{2}$/', $data['start_time']))
        $errors[] = 'Enter a valid start time.';
    if ($data['end_time'] !== '' && (!preg_match('/^\d{2}:\d{2}$/', $data['end_time']) || $data['end_time'] <= $data['start_time']))
        $errors[] = 'The end time must be after the start time.';
    if (!in_array($data['status'], ['draft', 'published', 'cancelled', 'postponed'], true))
    ...
    if ($admin) {
        ...
        if (!$check->fetchColumn())
            $errors[] = 'Select a valid organizer.';
    }
