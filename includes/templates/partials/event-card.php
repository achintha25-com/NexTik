...
<article class="event-card<?= $leadClass ?>">
    <a href="<?= e(app_url('event', ['id' => $event['id']])) ?>" class="event-card-media-link" aria-label="View <?= e($event['title']) ?>">
        ...
        <div class="event-card-content event-card-actions">
            ...
            <h3><a href="<?= e(app_url('event', ['id' => $event['id']])) ?>" class="event-card-title-link"><?= e($event['title']) ?></a></h3>
            <div class="event-card-footer">
                ...
                <?php else: ?>
                    <a href="<?= e(app_url('event', ['id' => $event['id']])) ?>" class="btn btn-primary btn-sm event-book-btn">View &amp; Book <span aria-hidden="true">→</span></a>
                <?php endif; ?>
            </div>
        </div>
</article>
