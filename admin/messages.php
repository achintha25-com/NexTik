...
<section class="section compact-top">
    <div class="container">
        <div class="admin-messages-heading">
            ...
            <span class="messages-count"><?= count($messages) ?>
                <?= count($messages) === 1 ? 'message' : 'messages' ?></span>
        </div>

        <?php if (!$messages): ?>
            <div class="empty-state-card">
                <h3>Your inbox is clear.</h3>
                <p>New messages from the Contact page will appear here.</p>
            </div>
        <?php else: ?>
                    ...
                    <article class="message-card status-<?= e($message['status']) ?>">
                        <div class="message-card-heading">
                            <div>
                                ...
                                <p class="message-meta"><a
                                        href="mailto:<?= e($message['email']) ?>"><?= e($message['email']) ?></a> ·
                                    <?= e(date('M j, Y · g:i A', strtotime($message['created_at']))) ?></p>
                            </div>
                            <span
                                class="status-badge status-<?= e($message['status']) ?>"><?= e(ucfirst($message['status'])) ?></span>
                        </div>
                        ...
                        <?php if (!empty($message['reply_text'])): ?>
                            <div class="message-reply">
                                <small>Reply
                                    saved<?= $message['replied_by_name'] ? ' by ' . e($message['replied_by_name']) : '' ?><?= $message['replied_at'] ? ' · ' . e(date('M j, Y · g:i A', strtotime($message['replied_at']))) : '' ?></small>
                        ...
                        <form class="message-reply-form" method="post" action="<?= e(app_url('admin-messages')) ?>">
                            ...
                            <label
                                for="reply-<?= e($message['id']) ?>"><?= !empty($message['reply_text']) ? 'Update your reply' : 'Write a reply' ?></label>
                            <textarea id="reply-<?= e($message['id']) ?>" name="reply_text" maxlength="5000"
                                required><?= e($message['reply_text'] ?? '') ?></textarea>
                            <div class="message-actions">
                                <button class="btn btn-primary btn-sm" type="submit">Save reply</button>
                                <a class="btn btn-secondary btn-sm"
                                    href="mailto:<?= e($message['email']) ?>?subject=<?= rawurlencode('Re: ' . $message['topic'] . ' - NexTik') ?>&body=<?= rawurlencode($message['reply_text'] ?? '') ?>">Email
                                    customer</a>
                            </div>
                        </form>
                    </article>
