    ...
    if ($action === 'delete') {
        if ($accountId === (int) $user['id'])
            ...
            if ($check->fetchColumn())
                flash('error', 'Accounts connected to events or bookings cannot be deleted.');
            else {
                db()->prepare('DELETE FROM users WHERE id = ?')->execute([$accountId]);
...
<section class="plain-page-head">
    <div class="container plain-page-head-inner">
        <div><span class="plain-page-label">Account management</span>
            <h1><?= e($accountPageTitle) ?></h1>
            <p>Review account activity and manage existing records.</p>
        </div>
    </div>
</section>
        ...
        <div class="admin-table-wrap">
            <table>
                ...
                <tbody><?php foreach ($accounts as $account): ?>
                        <tr>
                            <td><strong><?= e($account['name']) ?></strong><small
                                    class="table-subtitle"><?= e($account['email']) ?><?= $account['phone'] ? ' · ' . e($account['phone']) : '' ?></small>
                            </td>
                            <td><span class="status-badge"><?= e(ucfirst($account['role'])) ?></span></td>
                            <td><?= (int) $account['event_count'] ?> events · <?= (int) $account['booking_count'] ?> bookings
                            </td>
                            ...
                            <td>
                                <div class="table-actions">
                                    <form method="post"><?= csrf_field() ?><input type="hidden" name="action"
                                            ...
                                            value="<?= (int) $account['id'] ?>"><button class="btn btn-secondary btn-sm"
                                            type="submit">Save</button></form>
                                    <form method="post" data-confirm="Delete this account?"><?= csrf_field() ?><input
                                            type="hidden" name="action" value="delete"><input type="hidden" name="id"
                                            value="<?= (int) $account['id'] ?>"><button class="icon-btn icon-btn-danger"
                                            type="submit" aria-label="Delete account"
                                            <?= (int) $account['id'] === (int) $user['id'] ? 'disabled title="You cannot delete your own account"' : '' ?>><?= icon('delete') ?></button></form>
                                </div>
                            </td>
                        </tr><?php endforeach; ?>
                </tbody>
