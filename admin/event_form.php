...
if (is_post()) {
    ...
    [$data, $errors] = event_validation($_POST, true, $sold);
    $errors = array_merge($imageErrors, $errors);
    if ($errors === []) {
        $available = $data['total_tickets'] - $sold;
        if ($event) {
            db()->prepare('UPDATE events SET organizer_id=?, category_id=?, title=?, slug=?, description=?, venue=?, city=?, event_date=?, start_time=?, end_time=?, price=?, total_tickets=?, available_tickets=?, image=?, is_featured=?, status=? WHERE id=?')->execute([$data['organizer_id'], $data['category_id'], $data['title'], unique_slug($data['title'], $id), $data['description'], $data['venue'], $data['city'], $data['event_date'], $data['start_time'], $data['end_time'] ?: null, $data['price'], $data['total_tickets'], $available, $image, $data['is_featured'], $data['status'], $id]);
        } else {
            db()->prepare('INSERT INTO events (organizer_id, category_id, title, slug, description, venue, city, event_date, start_time, end_time, price, total_tickets, available_tickets, image, is_featured, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')->execute([$data['organizer_id'], $data['category_id'], $data['title'], unique_slug($data['title']), $data['description'], $data['venue'], $data['city'], $data['event_date'], $data['start_time'], $data['end_time'] ?: null, $data['price'], $data['total_tickets'], $data['total_tickets'], $image, $data['is_featured'], $data['status']]);
        }
