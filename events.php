    ...
    <div class="container event-detail-container">
        ...
        <header class="event-detail-heading">
            ...
            <div class="event-heading-meta" aria-label="Event schedule">
                ...
                <div><small>Starts</small><strong><?= e(date('g:i A', strtotime($event['start_time']))) ?></strong>
                </div>
            </div>
        </header>
        ...
        <div class="event-experience-grid">
            ...
            <article class="event-information-panel">
                ...
                <div class="event-facts">
                    <div>
                        <small>Date</small><strong><?= e(date('l, F j, Y', strtotime($event['event_date']))) ?></strong>
                    </div>
                    <div>
                        <small>Time</small><strong><?= e(date('g:i A', strtotime($event['start_time']))) ?><?= $event['end_time'] ? ' – ' . e(date('g:i A', strtotime($event['end_time']))) : '' ?></strong>
                    </div>
                    <div><small>Presented by</small><strong><?= e($event['organizer_name']) ?></strong></div>
                    <div><small>Tickets remaining</small><strong><?= number_format((int) $event['available_tickets']) ?>
                            / <?= number_format((int) $event['total_tickets']) ?></strong></div>
                </div>
            </article>
                    ...
                    <div class="ticket-option-list">
                        <?php foreach ($ticketOptions as $option): ?>
                            <div class="ticket-option-item">
                                ...
                                <small><?= number_format((int) $option['available_tickets']) ?> of
                                    <?= number_format((int) $option['total_tickets']) ?> left</small>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
                <?php if ($event['event_date'] < date('Y-m-d')): ?>
                    <div class="alert alert-error">This event has ended.</div><?php elseif (!$hasAvailability): ?>
                    <div class="alert alert-error">This event is sold out.</div><?php elseif (!$user): ?><a
                        class="btn btn-primary full-width" href="<?= e(app_url('login', ['role' => 'customer'])) ?>">Sign in
                        to book</a>
                    <p class="center muted">New customer? <a class="text-link" href="<?= e(app_url('register')) ?>">Register
                            here</a></p><?php elseif ($user['role'] === 'customer'): ?><a class="btn btn-primary full-width"
                        href="<?= e(app_url('book', ['id' => $event['id']])) ?>">Book tickets</a><?php else: ?>
                    <p class="muted">Ticket booking is available to customer accounts.</p><?php endif; ?>
                <div class="booking-assurance"><span>✓</span>
                    <p><strong>Simple booking</strong><small>Your booking details are confirmed immediately.</small></p>
                </div>
            </aside>
        </div>
    </div>
