    ...
    <div class="container login-content-grid">
        <div class="login-form-column">
            <section class="auth-wrapper">
                <div class="auth-card">
                    <p class="auth-role-label"><?= e(strtoupper($role)) ?> ACCOUNT</p>
                    ...
                    <div class="login-role-switcher" aria-label="Choose account type">
                        <a class="<?= $role === 'customer' ? 'active' : '' ?>"
                            href="<?= e(app_url('login', ['role' => 'customer'])) ?>">Customer</a>
                        <a class="<?= $role === 'organizer' ? 'active' : '' ?>"
                            href="<?= e(app_url('login', ['role' => 'organizer'])) ?>">Organizer</a>
                        <a class="<?= $role === 'admin' ? 'active' : '' ?>"
                            href="<?= e(app_url('login', ['role' => 'admin'])) ?>">Administrator</a>
                    </div>
                    <?php if ($errors): ?>
                    ...
                    <form method="post" action="<?= e(app_url('login', ['role' => $role])) ?>">
                        ...
                        <div class="form-group"><label for="password">Password</label>
                            <div class="password-field"><input class="form-control" id="password" name="password"
                                    ...
                                    class="password-toggle" data-password-toggle="password">Show</button></div>
                        </div>
                        <button class="btn btn-primary auth-submit" type="submit">Sign in</button>
                    </form>
                    <?php if ($role === 'customer'): ?>
                        <p class="auth-footer-text">New to NexTik? <a href="<?= e(app_url('register')) ?>">Create an
                                account</a></p><?php endif; ?>
                    <?php if ($role !== 'customer'): ?>
                        <p class="auth-footer-text">Organizer and administrator accounts are issued by the system
                            administrator.</p><?php endif; ?>
                </div>
            </section>
        </div>
