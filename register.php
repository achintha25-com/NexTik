...
if (is_post()) {
    ...
    if ($phone !== '' && mb_strlen($phone) > 20) $errors[] = 'Enter a valid phone number.';
    if (strlen($password) < 8) $errors[] = 'The password must contain at least 8 characters.';
    if ($password !== $confirmation) $errors[] = 'The password confirmation does not match.';
...
<p class="auth-role-label">CUSTOMER ACCOUNT</p><h1>Create your account</h1><p>Join NexTik and reserve your next experience.</p>
    ...
    <form method="post" action="<?= e(app_url('register')) ?>">
        <?= csrf_field() ?>
        <div class="form-group"><label for="name">Full name</label><input class="form-control" id="name" name="name" value="<?= e($_POST['name'] ?? '') ?>" required maxlength="255"></div>
        <div class="form-group"><label for="email">Email address</label><input class="form-control" id="email" name="email" type="email" value="<?= e($_POST['email'] ?? '') ?>" required></div>
        <div class="form-group"><label for="phone">Phone number</label><input class="form-control" id="phone" name="phone" value="<?= e($_POST['phone'] ?? '') ?>" maxlength="20"></div>
        <div class="form-group"><label for="password">Password</label><div class="password-field"><input class="form-control" id="password" name="password" type="password" minlength="8" required><button type="button" class="password-toggle" data-password-toggle="password">Show</button></div></div>
        <div class="form-group"><label for="password_confirmation">Confirm password</label><input class="form-control" id="password_confirmation" name="password_confirmation" type="password" minlength="8" required></div>
        <button class="btn btn-primary auth-submit" type="submit">Create account</button>
    </form>
    <p class="auth-footer-text">Already registered? <a href="<?= e(app_url('login', ['role' => 'customer'])) ?>">Sign in</a></p>
            </div></section>
        </div>
