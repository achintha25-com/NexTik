                            ...
                            <article class="organizer-event-row">
                                ...
                                <div class="organizer-event-details">
                                    <div class="organizer-event-title-row">
                                        ...
                                        <span
                                            class="status-badge status-<?= e($event['status']) ?>"><?= e(ucfirst($event['status'])) ?></span>
                                    </div>
                                    <div class="organizer-progress-line">
                                        <span><b><?= number_format($ticketsSold) ?></b> /
                                            <?= number_format((int) $event['total_tickets']) ?> tickets sold</span>
                                        <span><?= $soldPercentage ?>%</span>
                                    </div>
                                    <div class="organizer-progress"><span style="width: <?= $soldPercentage ?>%"></span></div>
                                </div>
                                <a class="icon-btn organizer-row-action"
                                    href="<?= e(app_url('organizer-event-form', ['id' => $event['id']])) ?>" title="Edit event"
                                    aria-label="Edit <?= e($event['title']) ?>">
                                    <?= icon('edit') ?>
                                </a>
                            </article>
                        <?php endforeach; ?>
                    ...
                    <div class="recent-booking-list">
                        <?php foreach ($recentBookings as $booking): ?>
                            <article class="recent-booking-row">
                                <div class="recent-booking-avatar">
                                    <?= e(mb_strtoupper(mb_substr($booking['customer_name'], 0, 1))) ?></div>
                                <div class="recent-booking-details">
                                    <h3><?= e($booking['customer_name']) ?></h3>
                                    <p><?= e($booking['event_title']) ?></p>
                                    <small><?= e($booking['booking_reference']) ?> &bull;
                                        <?= e(date('M j, g:i A', strtotime($booking['created_at']))) ?></small>
                                </div>
